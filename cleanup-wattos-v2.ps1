# WattOS_Plattform Bereinigungsskript v2
...
foreach ($key in $settingsToUpdate.Keys) {
    if ($settings.ContainsKey($key)) {
        if ($settings[$key] -ne $settingsToUpdate[$key]) {
            ...
            $updatedSettings += "$key`: $($settings[$key]) -> $($settingsToUpdate[$key])"
            $settings[$key] = $settingsToUpdate[$key]
            $updated++
        }
    } else {
        Write-Info "Hinzufüge: $key = $($settingsToUpdate[$key])"
        $updatedSettings += "$key`: (neu) -> $($settingsToUpdate[$key])"
        $settings[$key] = $settingsToUpdate[$key]
        $updated++
    }
}
...
Write-Info "Gesamtgröße: $([math]::Round($script:stats.BytesDeleted / 1MB, 2)) MB"
Write-Info "Freigegebener Speicherplatz: $([math]::Round($script:stats.BytesDeleted / 1MB, 2)) MB"
Write-Info "Fehler: $($script:stats.Errors)"
if ($updated -gt 0) {
    Write-Info "Optimierte Settings: $updated"
}
Write-Output ""
